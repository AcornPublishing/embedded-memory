...
subtime	DD		0
time	DD		0
TICS    DD      0                   ; temporary storage of the timer counter
Zero    DD  0
RESULTLIST  DD  ITER DUP (0)        ; list of the test results
...
BEGIN:  
    MOV     [COUNTER],0         ; Initializing iteration counter
...
TESTLOOP:                           ; loop check
	...
	sub eax, time 
	sub eax, subtime
    MOV     [TICS],EAX          ; | storing the results

	MOV     EDX,[COUNTER]       ; loop counter
	MOV     [RESULTLIST][EDX],EAX   ; storing the result in a table

	ADD     EDX,TYPE RESULTLIST     ; incrementing the counter
	MOV     [COUNTER],EDX           ; storing the timer counter
	CMP     EDX, ITER * (TYPE RESULTLIST)
	JB      TESTLOOP                ; repeating the iteration
